...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <form action="{{ route('admin.boarding-houses.update', $boardingHouse->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')

    {{-- Basic info --}}
    <input type="hidden" name="id" value="{{ $boardingHouse->id }}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="md:col-span-2 bg-white p-6 rounded-lg shadow-sm">
            <div class="space-y-4">
                <label class="block">
                    ...
                    <input type="text" name="name" value="{{ old('name', $boardingHouse->name) }}" required class="mt-1 block w-full rounded-md border-gray-200 shadow-sm"/>
                </label>

                <label class="block">
                    <span class="text-sm font-medium text-gray-700">Deskripsi</span>
                    <textarea name="description" rows="3" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm">{{ old('description', $boardingHouse->description) }}</textarea>
                </label>

                <label class="block">
                    <span class="text-sm font-medium text-gray-700">Alamat</span>
                    <input type="text" name="address" value="{{ old('address', $boardingHouse->address) }}" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm"/>
                </label>

                <div class="grid grid-cols-2 gap-4">
                    <label class="block">
                        <span class="text-sm font-medium text-gray-700">Harga / Bulan (Rp) <span class="text-red-500">*</span></span>
                        <input type="number" name="price" value="{{ old('price', $boardingHouse->price) }}" required class="mt-1 block w-full rounded-md border-gray-200 shadow-sm"/>
                    </label>
                    ...
                    <label class="block">
                        ...
                        @if($boardingHouse->image)
                            <img id="image-preview" src="{{ asset('storage/'.$boardingHouse->image) }}" alt="" class="mt-2 w-48 h-32 object-cover rounded-md border"/>
                        @else
        ...
        {{-- Campus distances card (pre-filled rows) --}}
        <div class="bg-white p-6 rounded-lg shadow-sm">
            ...
            <div id="campus-rows" class="space-y-3">
                {{-- loop existing campus distances --}}
                @if(!empty($campusDistances) && count($campusDistances))
                    @foreach($campusDistances as $row)
                        <div class="campus-row grid grid-cols-12 gap-2 items-center">
                            <div class="col-span-7">
                                <select name="campuses[]" class="w-full rounded-md border-gray-200 px-3 py-2">
                                    @foreach($campuses as $c)
                                        <option value="{{ $c->id }}" {{ (int)old('campuses.'.($loop->index), $row['id']) === $c->id ? 'selected' : '' }}>
                                            {{ $c->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-span-4">
                                <input type="number" name="distances[]" step="0.1" placeholder="Jarak (m)" value="{{ old('distances.'.($loop->index), $row['distance']) }}" class="w-full rounded-md border-gray-200 px-3 py-2" />
                            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            @foreach($c2 as $f)
                <label class="flex items-center gap-3 bg-gray-50 p-3 rounded-md">
                    <input type="checkbox" name="c2_{{ $f }}" id="c2_{{ $f }}" value="1"
                        class="h-4 w-4 text-primary-600 rounded"
                        {{ old('c2_'.$f, ($c2Map[$f] ?? 0)) ? 'checked' : '' }} />
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            @php
                $c4 = ['warung','laundry','klinik','atm','minimarket','fotocopy','tempat_ibadah','pasar'];
                $c4Map = $cvMap['C4'] ?? [];
            @endphp
            ...
            @foreach($c4 as $f)
                <div>
                    ...
                    <input type="number" step="0.1" name="c4_{{ $f }}" value="{{ old('c4_'.$f, $c4Map[$f] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-200 px-3 py-2" placeholder="meter">
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm">
        ...
        @php $c5Map = $cvMap['C5'] ?? []; @endphp
        <div class="grid grid-cols-3 gap-4">
            <label class="flex items-center gap-3 bg-gray-50 p-3 rounded-md">
                <input type="checkbox" name="c5_cctv" value="1" class="h-4 w-4 text-primary-600 rounded" {{ old('c5_cctv', ($c5Map['cctv'] ?? 0)) ? 'checked' : '' }}/>
                <span class="text-sm text-gray-700">CCTV</span>
            </label>
            <label class="flex items-center gap-3 bg-gray-50 p-3 rounded-md">
                <input type="checkbox" name="c5_pagar" value="1" class="h-4 w-4 text-primary-600 rounded" {{ old('c5_pagar', ($c5Map['pagar'] ?? 0)) ? 'checked' : '' }}/>
                <span class="text-sm text-gray-700">Pagar</span>
            </label>
            <label class="flex items-center gap-3 bg-gray-50 p-3 rounded-md">
                <input type="checkbox" name="c5_penjaga" value="1" class="h-4 w-4 text-primary-600 rounded" {{ old('c5_penjaga', ($c5Map['penjaga'] ?? 0)) ? 'checked' : '' }}/>
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="c6_jam_malam" class="mt-1 block w-full rounded-md border-gray-200 px-3 py-2">
                    <option value="ya" {{ old('c6_jam_malam', ($c6Map['jam_malam'] ?? '') === 1 ? 'ya' : (old('c6_jam_malam') == 'ya' ? 'ya' : '')) ? 'selected' : '' }}>Ya</option>
                    <option value="tidak" {{ old('c6_jam_malam', ($c6Map['jam_malam'] ?? '') === 0 ? 'tidak' : '') ? 'selected' : '' }}>Tidak</option>
                </select>
            ...
            <div>
                ...
                <div class="mt-2">
                    <label class="inline-flex items-center gap-2">
                        <input type="checkbox" name="c6_membawa_teman" value="1" class="h-4 w-4 text-primary-600 rounded" {{ old('c6_membawa_teman', ($c6Map['membawa_teman'] ?? 0)) ? 'checked' : '' }}/>
            ...
            <div>
                ...
                <select name="c6_ketentuan_bayar" class="mt-1 block w-full rounded-md border-gray-200 px-3 py-2">
                    <option value="1" {{ old('c6_ketentuan_bayar', $c6Map['ketentuan_bayar'] ?? '') == 1 ? 'selected' : '' }}>1 bulan</option>
                    <option value="3" {{ old('c6_ketentuan_bayar', $c6Map['ketentuan_bayar'] ?? '') == 3 ? 'selected' : '' }}>3 bulan</option>
                    <option value="6" {{ old('c6_ketentuan_bayar', $c6Map['ketentuan_bayar'] ?? '') == 6 ? 'selected' : '' }}>6 bulan</option>
                    <option value="12" {{ old('c6_ketentuan_bayar', $c6Map['ketentuan_bayar'] ?? '') == 12 ? 'selected' : '' }}>12 bulan</option>
                </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <label class="flex items-center gap-3 bg-gray-50 p-3 rounded-md">
                <input type="checkbox" name="c6_tamu_menginap" value="1" class="h-4 w-4 text-primary-600 rounded" {{ old('c6_tamu_menginap', ($c6Map['tamu_menginap'] ?? 0)) ? 'checked' : '' }}/>
                <span class="text-sm text-gray-700">Tamu Menginap</span>
            </label>

            <label class="flex items-center gap-3 bg-gray-50 p-3 rounded-md">
                <input type="checkbox" name="c6_hewan_peliharaan" value="1" class="h-4 w-4 text-primary-600 rounded" {{ old('c6_hewan_peliharaan', ($c6Map['hewan_peliharaan'] ?? 0)) ? 'checked' : '' }}/>
